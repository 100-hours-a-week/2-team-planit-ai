...
3. 출력 규칙(엄격):
   ...
   - 만약 입력이 부족하거나 전혀 유추할 수 없다면, <final_response>에는 정확히 "정보 불충분" 만을 한 문장(문구)으로 반환하세요.
...
5. 스타일 가이드:
   ...
   - 예시적 표현: "편안한 힐링을 최우선으로 하는 30대 부부 여행자, 예산과 시간 제약이 있어 근교 숙박 위주를 선호하며 성공 기준은 충분한 휴식(온천, 조용한 카페)입니다." — 위 문장은 예시일 뿐, 실제 응답은 입력에 맞춰 축약하고 하나의 문장으로 만드세요.
